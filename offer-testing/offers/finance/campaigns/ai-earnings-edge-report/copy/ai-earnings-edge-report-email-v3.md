# Email Copy: AI Earnings Edge Report - Version 3

> **Campaign:** AI Earnings Edge Report  
> **Version:** 3 (DIY Guide - Build It Yourself)  
> **Length:** ~80 words  
> **Goal:** Get reply requesting DIY guide

---

## Subject Line Options

1. "build earnings AI" (RECOMMENDED)
2. "DIY earnings analysis"
3. "portfolio-wide AI"

---

## Email Body

Subject: build earnings AI

{FirstName},

Most funds analyze earnings calls one at a time.

What if AI could do it for every company in your portfolio simultaneously?

We built a guide showing how to set this up yourself. No vendor dependency, runs on your schedule, scales to hundreds of companies.

A hedge fund client of ours expanded the number of companies in their research universe 8x using this approach.

Want it? Just reply "send guide".

Best,
Mike

---

## Personalization Notes

- {FirstName}: Contact's first name only
- Keep everything plain text, no bold

---

## Follow-up (If No Response After 3 Days)

Subject: Re: build earnings AI

Still happy to send the guide—shows you how to automate earnings analysis for your entire portfolio.

Zero strings.

Best,
Mike
